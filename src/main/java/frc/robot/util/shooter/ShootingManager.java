...
/**
 * ShootingManager - Coordinates ballistic calculations and motor adjustment computations to produce
 * the runtime shooting parameters used by shooter control.
 *
 * <p>This class is a static utility that maintains the most recently computed shooting parameters
 * (angles, launch velocity, and desired motor acceleration). It performs the following high-level
 * steps when updating shooting parameters: - Predict the robot-relative distance and angle to the
 * hub after the reload delay - Compute ballistic trajectory parameters needed to hit the hub -
 * Compute motor adjustments required to restore launcher wheel energy after firing
 *
 * <p>The class stores results in public static fields and exposes simple getters for other
 * subsystems or commands. All fields are updated by {@link #updateShootingParameters}.
 */
public class ShootingManager {
   ...
   * abstracting away the conversion details. The conversion factors are defined in {@code
   * Constants} and are based on the physical design of the shooter mechanisms, (e.g., how many
   * motor revolutions correspond to one degree of hood pitch or turret yaw).
   */
    ...
    public static Pose2d getShootingTarget(Pose2d pose) {
      ...
      if (allianceOpt.get() == DriverStation.Alliance.Blue) {
        ...
        if (pose.getX() < FieldConstants.NEUTRAL_ZONE_REDSIDE) {
          return isTop
              ? FieldConstants.RED_TOP_FERRY_TARGET
